...
El admin es el único rol que accede al módulo de [Tenants](/modulos/tenants):

- Plan y límites (`maxWorkers`, `limiteObras`).
- Módulos activos (`settings.modulosActivos`).
- Reglas SST (`reglas.fasesObligatorias`, `requiereFirmaPin`).
- Personalización UI (colores, logo, preferencias regionales).
